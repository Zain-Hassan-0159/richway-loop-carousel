 ...
 * @since 1.0.0
 */
class richway_Loop_Carousel extends Widget_Base {
	...
	public function get_title() {
		return __( 'Richway loop Carousel', 'hub-elementor-addons' );
	}
	...
	private function get_posts_titles_ids() {
		...
		foreach ($post_types as $post_type) {
			...
			foreach ($posts as $post) {
				$posts_array[$post->ID] = esc_html__($post->post_title, 'hub-elementor-addons');
			}
		}
	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'options' => [
					...
					'post_type_content'  => esc_html__( 'Post Type', 'hub-elementor-addons' ),
				],
			]
		);
		...
		$this->add_control(
			'no_of_posts',
			[
				'label' => esc_html__( 'No of Posts', 'hub-elementor-addons' ),
		...
		$this->start_controls_tab(
			'include',
			[
				'label' => esc_html__( 'Include', 'hub-elementor-addons' ),
				'condition' => [
					'carousel_type' => 'post_type_content'
				]
			]
		...
		$this->add_control(
			'include_posts',
			[
				'label' => esc_html__( 'Select Posts to Include', 'hub-elementor-addons' ),
		...
		$this->start_controls_tab(
			'exclude',
			[
				'label' => esc_html__( 'Exclude', 'hub-elementor-addons' ),
				'condition' => [
					'carousel_type' => 'post_type_content'
				]
			]
		...
		$this->add_control(
			'exclude_posts',
			[
				'label' => esc_html__( 'Select Posts to Exclude', 'hub-elementor-addons' ),
				'type' => Controls_Manager::SELECT2,
				'label_block' => true,
				'multiple' => true,
				'options' => $this->get_posts_titles_ids(),
		...
		$this->add_control(
			'order_by',
			[
				'label' => esc_html__( 'Order By', 'hub-elementor-addons' ),
				'type' => Controls_Manager::SELECT,
				'options' => [
					'title' => 'Title',
					'rand' => 'Random',
		...
		$this->add_control(
			'order',
			[
				'label' => esc_html__( 'Order', 'hub-elementor-addons' ),
				'type' => Controls_Manager::SELECT,
				'options' => [
					'ASC' => 'Ascending',
					'DESC' => 'Descending',
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}}' => 'width: {{SIZE}}{{UNIT}}; flex: 0 0 auto;',
				],
	...
	protected function limitString($string, $limit = 250, $append = '...') {
			...
			<div class="carousel-items pos-rel <?php echo implode(' ', $carousel_items_classes); ?>" <?php self::get_options(); ?>>
				...
				<div class="flickity-viewport pos-rel w-100 <?php echo $viewport_overflow; ?>">
					<div class="<?php echo ld_helper()->sanitize_html_classes( $slider_element_classnames ); ?>" style="<?php echo $origin ?>: 0; transform: translateX(0%);">
						...
						<?php }
						} elseif($settings['carousel_type'] === 'post_type_content') {
							...
							$the_query = new WP_Query( 
								array( 
								  ...
								  'post__in' => $settings['include_posts'], // Array of post IDs to include
								  'post__not_in' =>$settings['exclude_posts'], // Array of post IDs to exclude 
								) 
							);
							...
							if( $the_query->have_posts() ) {
								while( $the_query->have_posts() ) {
									...
									if ( ! empty($settings['item_width_set']['size'] ) ) {
										array_push($item_classnames, 'width-is-set');
									}
									?>
									<div class="<?php echo ld_helper()->sanitize_html_classes( $item_classnames ); ?>">
										<div class="carousel-item-inner pos-rel w-100">
											<div class="carousel-item-content pos-rel w-100">
												<?php echo \Elementor\Plugin::instance()->frontend->get_builder_content( $settings['templatess'], true ); ?>
											</div>
										</div>
									</div>
									<?php 
